...
//
// Each tool below is `nonisolated` (required to satisfy FoundationModels'
// Tool protocol conformance — the LLM runtime calls these off the main
// actor), but `call(arguments:)` is `async`, so its actual SwiftData work
// hops to `await MainActor.run { ... }` and reads through
// `modelContainer.mainContext` — the same single, already-serialized
// context the rest of the app uses — rather than creating a second,
// independent `ModelContext` per call. A second context bound to the same
// store is exactly the kind of setup that produces cross-context object
// invalidation ("this model instance was invalidated...") if a delete
// lands on one context while the other still holds a fetched reference to
// the same row; routing through the one shared context removes that
// concurrency surface entirely instead of trying to manage it.
...
nonisolated struct SpendSummaryTool: Tool {
    ...
    func call(arguments: Arguments) async throws -> String {
        ...
        // Hops to the app's one shared, already-serialized context instead
        // of standing up a second ModelContext — see FinanceTools' header
        // note on why. Everything that touches SwiftData model objects
        // stays inside this closure; only the final Sendable String leaves it.
